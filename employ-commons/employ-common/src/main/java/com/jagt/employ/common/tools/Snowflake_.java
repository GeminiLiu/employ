...
public class Snowflake_ {
    ...
    // ============================== Constants===========================================
    private final static String ERROR_CLOCK_BACK = "Clock moved backwards.  Refusing to generate id for %d milliseconds";
    
    private final static String ERROR_ATTR_LIMIT = "%s can't be greater than %d or less than 0";
    ...
    /** 序列在id中占的位数 (表示只允许sequenceId的范围为：0-1023)*/
     ...
     */
    public Snowflake_() {
        this.workerId = new Random().nextInt((int) maxWorkerId+1);
        this.datacenterId = new Random().nextInt((int) maxDatacenterId+1);
    }
    /**
     * 构造函数
     * 
     * @param workerId 工作ID (0~511)
     * @param datacenterId 数据中心ID (0~15)
     */
    public Snowflake_(long workerId, long datacenterId) {
